...
def build_single_note_variation_payload(state_keys: List[str], name: str, letter: str, measures: List[List[str]]):
    ...
    return {'id': letter, 'name': name, 'score': 0.0, 'state_keys': state_keys, 'chords': chords}
...
def generate_two_variations_from_measures(measures: List[List[str]]):
    ...
    if all_single_note_measures(measures):
        ...
        # Keep a second variation for UI consistency, but make A the exact attached-style canon output.
        ...
        if not general_paths:
            data['variation_b'] = build_single_note_variation_payload(single_path, 'Variation B', 'B', measures)
        else:
            data['variation_b'] = build_variation_payload(general_paths[0], 'Variation B', 'B', measures)
        return data
